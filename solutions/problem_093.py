...
# Warning: Runtime ~ 22s
def compute(): # Solved by finding all reachable numbers for a given set of digits by making all possible evaluation trees
	OPS = [(operator.add, False), (operator.sub, False), (operator.sub, True), \
		(operator.mul, False), (operator.div, False), (operator.div, True)]  # boolean is 'reverse', i.e. do (right op left) instead
	# Represent computations options as a tree: leaves are int, non-leaves are operations on their child subtrees
	def generate_trees(digits):  # digits is a set or a tuple of digits
		...
		# Map each subset of digits to list of possible subtrees, be sure to skip the 'full' subset or we get infinite recursion
		subtrees = {frozenset(subset):generate_trees(subset) \
			for subset in utils.powerset(digits) if len(subset) != len(digits)} 
		results = []
		for subset in filter(lambda s: min(digits) in s, subtrees.keys()): # Iterate through subsets containing smallest elt
	...
	best = max((lowest_impossible(digits), digits) for digits in itertools.combinations(range(10), 4))
	a = utils.concatenate_ints(sorted(best[1]))
	return a, 'Four digits that allow for highest range of reachable values (up to %d)' % (best[0]-1,)
